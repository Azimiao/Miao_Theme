    ...
    if(has_post_thumbnail()){ 
       ...
       the_post_thumbnail();
        ...
        if(empty($post_img_src)){ 
			...
			$post_img = get_bloginfo('template_directory').'/images/random/tb'.$random.'.jpg';
...
function comments_list($comment, $args, $depth) {
  ...
  if(!$parent_id = $comment->comment_parent) {
    ...
    echo '<div class="coms_floor"><a href="#comment-'.get_comment_ID().'">'; printf('%1$s楼', --$commentcount); echo '</a></div>';
  ...
  echo '<div class="coms_main" id="div-comment-'.get_comment_ID().'">';
    ...
    echo '<div class="coms_meta">';
      ...
      echo '<span class="coms_author"><a href="'; echo comment_author_url(); echo '" target="_blank" rel="external nofollow" class="url">'; echo comment_author(); echo get_author_class($comment->comment_author_email,$comment->user_id); if(user_can($comment->user_id, 1)){echo "<a title='博主认证' class='vip'></a>";}; echo '</a></span>'; echo get_comment_time('m-d H:i '); echo time_ago(); 
